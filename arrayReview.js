...
function removeEven(arr) {
  ...
  for (var i in arr) {
    if (evenArray[i] % 2 === 0) {
      newArray.push(evenArray[i]);
    }
  }
  return newArray;
}
console.log(removeEven(evenArray));
...
function checkRandom(randomNum) {
...
checkRandom(getRandomArbitrary());
...
function longest(str) {
  ...
  for (var i in tempStr) {
    if (tempStr[i].length > max) {
      max = tempStr[i].length;
      maxIndex = i;
    }
  }
  console.log(tempStr[maxIndex]);
